...
- **Parsing**: `parse_exonerate_gff` reads verbatim `--showtargetgff yes` GFF (one result per `START…END OF GFF DUMP` block) + a `--ryo` line whose `%tcs` is the authoritative spliced CDS. **`%tcs` is the emitted CDS** (`build_result_from_model`, D-039), with the coord-rebuilt CDS kept only as a fallback when `%tcs` is absent and as a `tcs_matches` cross-check — the coordinate rebuild can slip a few bp at a split-codon/intron-phase boundary and frameshift the whole CDS into bogus internal stops (the cause of spurious RPB2/TUB2 `REVIEW` verdicts). `cds_source` (`tcs`|`coords`) is logged. Exonerate emits introns + splice5/splice3 explicitly (no gap inference). `--bestn N` surfaces paralogs (`select_best_model` returns `(best, others)`).
